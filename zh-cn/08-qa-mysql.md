...
- 主要在两方面
    - MyISAM 的 **数据存储方式** 使用非聚集索引，MYISAM 存储的数据是具体记录行的地址，直接定位到文件offset，查询的时候不需要回表，而 InnoDB 使用聚集索引，普通索引（普通索引、二级索引、辅助索引是同个东西）查询的时候需要回表（普通索引>主键，然后回表查主键索引>数据记录），所以 myisam 定位记录行更快。
    - 同时 innodb 每次查询都需要维护一个 **mvcc** 数据版本的一致情况，所以整体上 myisam 存储引擎的设计更具备查询优势。 
